...
// DEBUG: Enhanced session tracking with connection timestamps
...
function logActivity(action, socketId, data = {}) {
  ...
  // DEBUG: Track session activities
  if (socketId) {
...
io.on('connection', (socket) => {
  ...
  socket.on('join-room-as-professor', (data) => {
    try {
      console.log(`👨‍🏫 JOIN-ROOM-AS-PROFESSOR attempt from socket ${socket.id.substring(0, 8)}`);
      console.log(`   Request data:`, data);
      
      const { roomKey: inputRoomKey, professorName, createIfNotExists = true } = data;

      if (!professorName || professorName.trim().length === 0) {
        console.error(`❌ Invalid professor name: ${professorName}`);
      ...
      console.log(`   Normalized room key: ${normalizedRoomKey}`);
      ...
      if (!rooms.has(normalizedRoomKey)) {
        if (createIfNotExists) {
          ...
          console.error(`❌ Room ${normalizedRoomKey} does not exist and createIfNotExists=false`);
          socket.emit('room-error', { 
      ...
      if (room.professors.size >= 3) {
        console.error(`❌ Professor limit reached for room ${normalizedRoomKey}`);
        socket.emit('room-error', { 
      ...
      console.log(`   Socket joined room ${normalizedRoomKey}`);
      ...
      console.log(`   Professor added to room data`);
  ...
  socket.on('draw-command', (data) => {
    try {
      ...
      if (!userSession) {
        console.error(`❌ PERMISSION DENIED: User session not found for socket ${socket.id.substring(0, 8)}`);
      ...
      if (userSession.role !== 'professor') {
        console.error(`❌ PERMISSION DENIED: User ${userSession.name} (${userSession.role}) attempted to draw`);
        socket.emit('permission-denied', { 
      ...
      if (!room) {
        console.error(`❌ Room ${roomKey} not found for drawing command`);
      ...
      console.error(`❌ Error processing draw command from ${socket.id.substring(0, 8)}:`, error);
      socket.emit('error', { 
        message: 'Failed to process drawing command', 
        error: error.message 
      });
    }
  });
  ...
  socket.on('disconnect', (reason) => {
    try {
      console.log(`🔌 DISCONNECT: Socket ${socket.id.substring(0, 8)} disconnected. Reason: ${reason}`);
      ...
      if (userSession) {
        ...
        console.log(`   Session found: ${role} ${name} in room ${roomKey}`);
        
        if (room) {
          // Remove user from appropriate collection
          if (role === 'professor') {
            const wasRemoved = room.professors.delete(socket.id);
            console.log(`   Professor removed from room: ${wasRemoved}`);
          } else {
            const wasRemoved = room.students.delete(socket.id);
            console.log(`   Student removed from room: ${wasRemoved}`);
          }
          ...
          console.log(`   Remaining users in room: ${totalUsers}`);
            ...
            setTimeout(() => {
              if (rooms.has(roomKey)) {
                ...
                if (roomCheck.professors.size === 0 && roomCheck.students.size === 0) {
                  ...
                  console.log(`   🧹 Room ${roomKey} deleted due to inactivity`);
                }
              }
        ...
        console.log(`   Session deleted for socket ${socket.id.substring(0, 8)}`);
      } else {
        console.log(`   No session found for socket ${socket.id.substring(0, 8)}`);
        console.log(`   Available sessions:`, Array.from(userSessions.keys()).map(id => id.substring(0, 8)));
      }
      ...
      console.error(`❌ Error handling disconnect for ${socket.id.substring(0, 8)}:`, error);
    }
  });
...
app.get('/debug/sessions', (req, res) => {
  const sessionList = Array.from(userSessions.entries()).map(([socketId, session]) => ({
    socketId: socketId.substring(0, 8),
    role: session.role,
    name: session.name,
    roomKey: session.roomKey,
    userId: session.userId ? session.userId.substring(0, 8) : 'N/A'
  }));
...
app.get('/debug/sessions/:socketId', (req, res) => {
  ...
  const debugLog = debugSessionLog.get(session.userId || targetSocketId);
  
  res.json({
    session: {
      socketId: session.userId ? session.userId.substring(0, 8) : 'N/A',
      role: session.role,
      name: session.name,
      roomKey: session.roomKey
    },
...
// DEBUG: Get all debug session logs
app.get('/debug/logs', (req, res) => {
  const logs = Array.from(debugSessionLog.entries()).map(([socketId, log]) => ({
    socketId: socketId.substring(0, 8),
    connectTime: log.connectTime,
    lastActivity: log.lastActivity,
    eventCount: log.events.length,
    recentEvents: log.events.slice(-10) // Last 10 events
  }));
  
  res.json({
    logs: logs,
    totalLogs: debugSessionLog.size,
    timestamp: new Date().toISOString()
  });
});
...
// DEBUG: Temporary permission bypass endpoint (FOR TESTING ONLY)
app.post('/debug/bypass-permissions/:socketId', (req, res) => {
  ...
  if (!fullSocketId) {
    return res.status(404).json({ error: 'Socket session not found' });
  }
  
  // Force set role to professor
